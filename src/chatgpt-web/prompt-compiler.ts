...
export function compileWebPrompt(input: CompileWebPromptInput): CompiledWebPrompt {
  ...
  const payload = {
    ...
    outputContract: {
      ...
      proposePatchRule: "For each PROPOSE_PATCH intent, set patch to exactly @@ISEOL_PATCH:<intentId>@@ inside the single-line JSON header. After the JSON, emit exactly one raw patch appendix for that intent using @@ISEOL_PATCH_BEGIN:<intentId>@@ and @@ISEOL_PATCH_END:<intentId>@@. Each appendix must contain exactly one file unified diff and be git apply-compatible using one diff --git / --- / +++ file header pair and valid @@ hunks. Every hunk body line needs a unified-diff prefix: space for context, + for additions, - for deletions, or \\ for the no-newline marker; a blank added line must be a single +. Hunk range counts must match their body lines. Never use *** Begin Patch, *** Update File, *** Add File, or *** Delete File markers.",
      proposePatchExample: {
