...
export function calendarQuickConnectPlan(project: StoredProject, oauthReady: boolean): CalendarQuickConnectPlan {
...
function looksLikeAdminPermissionError(error: string): boolean {
  ...
  return ["403", "forbidden", "permission", "resource not accessible", "workflow"]
    .some((keyword) => normalized.includes(keyword));
}

async function resolveLogChannel(interaction: ButtonInteraction, project: StoredProject, storedId: string | undefined, name: string): Promise<TextChannel> {
  ...
  const existing = channels.find((channel) => channel instanceof TextChannel && channel.parentId === project.categoryId && channel.name === name);
  if (existing instanceof TextChannel) return existing;
  const created = await guild.channels.create({ name, type: ChannelType.GuildText, parent: project.categoryId, reason: `${project.name} GitHub 로그 연동 자동 복구` });
...
async function ensureDiscordWebhookUrl(channel: TextChannel, name: string): Promise<string> {
  const existing = (await channel.fetchWebhooks()).find((webhook) => webhook.name === name && Boolean(webhook.url));
  if (existing?.url) return existing.url;
  const created = await channel.createWebhook({ name, reason: `${name} GitHub integration` });
  return created.url;
}

async function ensureRepositoryLogHook(github: GitHubWebhookService, repository: RepositoryRef, channel: TextChannel, webhookName: string): Promise<{ id: number; created: boolean }> {
...
async function ensureGitHubProjectHooks(interaction: ButtonInteraction, project: StoredProject, github: GitHubWebhookService, result: QuickConnectResult): Promise<StoredProject> {
  try {
    const frontendLog = await resolveLogChannel(interaction, project, project.frontendLogChannelId, "💻・frontend-log");
    const backendLog = await resolveLogChannel(interaction, project, project.backendLogChannelId, "🛠・backend-log");
    ...
    if (frontendHook.created || backendHook.created || !project.frontendHookId || !project.backendHookId) result.connected.push("GitHub 프로젝트");
    else result.unchanged.push("GitHub 프로젝트");
    return updated;
  } catch (error) {
...
async function ensureReview(github: GitHubWebhookService, project: StoredProject, result: QuickConnectResult): Promise<void> {
  try {
    ...
    for (const review of reviews) {
      ...
      if (looksLikeAdminPermissionError(review.error)) {
        if (!result.needsAdmin.includes("Code Review · GitHub Workflows 권한")) result.needsAdmin.push("Code Review · GitHub Workflows 권한");
      } else if (!result.failed.includes("Code Review")) result.failed.push("Code Review");
    }
    if (healthy && !result.needsAdmin.some((item) => item.startsWith("Code Review")) && !result.failed.includes("Code Review")) {
      (changed ? result.connected : result.unchanged).push("Code Review");
    }
  } catch (error) {
    console.error(`빠른 Code Review 연동 실패 (${project.name})`, error);
    const detail = error instanceof Error ? error.message : String(error);
    if (looksLikeAdminPermissionError(detail)) result.needsAdmin.push("Code Review · GitHub Workflows 권한");
    else result.failed.push("Code Review");
  }
}

async function ensureCalendar(interaction: ButtonInteraction, project: StoredProject, result: QuickConnectResult): Promise<{ project: StoredProject; authorizationUrl?: string }> {
  ...
  const action = googleCalendarConnectAction({ hasCalendar: Boolean(project.calendarId), clientId: config.googleClientId, clientSecret: config.googleClientSecret, publicBaseUrl: redirectUri, refreshToken });
  if (action === "connected") {
  ...
  if (action === "authorize") {
    const session = createGoogleOAuthSession({ projectId: project.id, guildId: project.guildId, userId: interaction.user.id });
    const authorizationUrl = buildGoogleAuthorizationUrl({ clientId: config.googleClientId, clientSecret: config.googleClientSecret, redirectUri, state: session.state });
    result.needsAction ??= [];
    result.needsAction.push("Google Calendar · Google 로그인 1회");
    return { project, authorizationUrl };
  }
  try {
    const created = await new GoogleCalendarService(config.googleClientId, config.googleClientSecret, refreshToken, redirectUri).createProjectCalendar(project.name);
    const updated = await updateProject(project.id, { calendarId: created.id, calendarUrl: created.url }) ?? project;
...
async function ensureDiscordExperience(interaction: ButtonInteraction, project: StoredProject, result: QuickConnectResult): Promise<StoredProject> {
  try {
    const before = [project.hubPanelMessageId, project.scrumChannelId, project.scrumPanelMessageId, project.calendarChannelId, project.calendarPanelMessageId].join(":");
    await ensureProjectExperience(interaction.client, project);
    const latest = await findProject(project.id) ?? project;
    const after = [latest.hubPanelMessageId, latest.scrumChannelId, latest.scrumPanelMessageId, latest.calendarChannelId, latest.calendarPanelMessageId].join(":");
...
export async function handleProjectConnectButton(interaction: ButtonInteraction): Promise<boolean> {
  ...
  if (parsed.action === "github_help") {
    await interaction.reply(projectGitHubPermissionGuide(project));
    return true;
  }
  if (!interaction.memberPermissions?.has(PermissionFlagsBits.ManageChannels)) {
    await interaction.reply({ content: "프로젝트 연동 변경은 프로젝트 관리 권한이 있는 사용자만 할 수 있습니다.", ephemeral: true });
    return true;
  }

  await interaction.deferReply({ ephemeral: true });
  const result: QuickConnectResult = { connected: [], unchanged: [], needsAdmin: [], failed: [] };
  const github = new GitHubWebhookService(config.githubToken);
  let authorizationUrl: string | undefined;

  if (parsed.action === "auto") project = await ensureDiscordExperience(interaction, project, result);
  ...
  const content = [
    ...
    result.needsAdmin.some((item) => item.startsWith("Code Review")) ? "🔐 빠른 연동 화면의 `GitHub 권한 안내`에서 필요한 권한을 바로 확인할 수 있습니다." : null,
    hubRefreshed ? "✅ 프로젝트 허브 상태도 갱신했습니다." : "⚠️ 허브 상태 갱신은 다음 재시작 때 다시 시도합니다.",
  ].filter((value): value is string => Boolean(value)).join("\n\n");
  const components = authorizationUrl
    ? [new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setLabel("Google Calendar 연결").setEmoji("📅").setStyle(ButtonStyle.Link).setURL(authorizationUrl))]
    : [];
